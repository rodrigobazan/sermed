...
public class ModificarPlanAfiliadoIntegrationTest {

    private String url ="http://localhost:8080";

    @Test
    public void cambiarPlan_PlanDistinto_Devuelve200() throws Exception {
        JSONObject envoltura = new JSONObject();
        envoltura.put("afiliado", factoryAfiliado());
        envoltura.put("plan", factoryNuevoPlan());
        String token = TokenAuthentication.obtainAccessToken("usuario", "123456");
        Header header = new BasicHeader("Authorization", "Bearer "+token);
        HttpPost post = new HttpPost(url+"/sermed/afiliado/modificarPlan");
    ...
    @Test
    public void cambiarPlan_PlanIgual_Devuelve412() throws Exception {
        JSONObject envoltura = new JSONObject();
        envoltura.put("afiliado", factoryAfiliado());
        envoltura.put("plan", factoryPlan());
        String token = TokenAuthentication.obtainAccessToken("usuario", "123456");
        Header header = new BasicHeader("Authorization", "Bearer "+token);
        HttpPost post = new HttpPost(url+"/sermed/afiliado/modificarPlan");
        StringEntity se = new StringEntity(envoltura.toString());
    ...
    private JSONObject factoryNuevoPlan() throws JSONException {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("idPlan", 2);
        jsonObject.put("nombrePlan", "Nuevo Plan Modificado");
        jsonObject.put("listaPrecios", new JSONObject(factoryListaPreciosPlan()));
        return jsonObject;
    }

    private HashMap<String, Double> factoryListaPreciosPlan(){
        try {
            HashMap<String, Double> listaPrecios = new HashMap<>();
            listaPrecios.put("1", (double) 400);
            listaPrecios.put("2", (double) 500);
            listaPrecios.put("3", (double) 600);
            listaPrecios.put("4", (double) 7000);
            return listaPrecios;
        }catch (Exception e){
            e.printStackTrace();
            return new HashMap<>();
